...
# endif
  ...
  while(std::getline(std::cin, s, '\n')) {
    ...
    for(int i = 0; i < (step < 0 ? M.size() / 2 : M.size()) - 1; i ++)
      std::cout << (chain ? (step < 0 ? bM[i] + d[i + M.size() / 2] : bM[i]) : M[i]) << ", ";
    std::cout << (chain ? (step < 0 ? bM[M.size() / 2 - 1] + d[M.size() / 2 + M.size() / 2 - 1] : bM[bM.size() - 1]) : M[M.size() - 1]) << std::endl << std::flush;
    M = MM.res[0];
  }
#if !defined(_ONEBINARY_)
  return 0;
