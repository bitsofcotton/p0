...
int main(int argc, const char* argv[]) {
  ...
  if(look < 0) {
    if(var < 0)
      qq = p0_ast(p0_st(p0_s4t(p0_s3t(p0_s2t(p0_1t(p0_0t(step, abs(var * 2) + abs(look) - 1), abs(var)) )) ), abs(var)));
    else
      pp = p0_st(p0_s4t(p0_s3t(p0_s2t(p0_1t(p0_0t(step, abs(var * 2) + abs(look) - 1), abs(var)) )) ), abs(var));
  } else {
    if(var < 0)
      q = p0_at(p0_t(p0_8t(p0_7t(p0_6t(p0_5t(p0_4t(p0_3t(p0_2t(p0_1t(p0_0t(step, abs(var * 2) + abs(look) - 1), abs(var)), step), step), step), step) )) ), abs(var)) );
    else
      p = p0_t(p0_8t(p0_7t(p0_6t(p0_5t(p0_4t(p0_3t(p0_2t(p0_1t(p0_0t(step, abs(var * 2) + abs(look) - 1), abs(var)), step), step), step), step) )) ), abs(var));
  }
  num_t din(int(0));
  auto  Mx(din);
  std::vector<num_t> M;
  ...
  while(std::getline(std::cin, s, '\n')) {
    ...
    for(int i = 0; i < 2; i ++) {
      ...
      if(i) std::cout << D << ", " << M[M.size() - 1] << std::endl;
      d1 = d0;
    }
  }
  return 0;
